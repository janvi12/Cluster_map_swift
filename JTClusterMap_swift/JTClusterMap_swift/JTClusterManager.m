...
@implementation JTClusterManager
...
+ (NSArray *) clusterAnnotationsForMapView:(MKMapView *)mapView forAnnotations:(NSArray *)pins blocks:(NSUInteger)blocks minClusterLevel:(NSUInteger)minClusterLevel
{
    ...
    for ( ; i < length ; i ++ )
    {
        JTClusterBlock *block = [[JTClusterBlock alloc] init];
    ...
    for (JTClusterPin *pin in visibleAnnotations)
    {
        ...
        [(JTClusterBlock *)[clusteredBlocks objectAtIndex:localTileNumber] addAnnotation:pin];
    }
    
    //create New Pins
    NSMutableArray *newPins = [NSMutableArray array];
    for ( JTClusterBlock *block in clusteredBlocks )
    {
        if( [block count] > 0 )
        {
            if( ![JTClusterManager clusterAlreadyExistsForMapView:mapView andBlockCluster:block] )
            {
...
{
    for ( JTClusterPin *pin in mapView.annotations )
    {
        if( [pin isKindOfClass:[JTClusterPin class]] && [[pin nodes] count] > 0 )
        {
